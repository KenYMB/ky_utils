classdef ViolinHalf < handle
    ...
    methods
        function obj = ViolinHalf(data, pos, varargin)
            ...
            %     'Side'         Which side to plot the violin.
            %                    'left'(default),'right','both'
            ...
            if strcmpi(args.Side,'right')
                widthL = 0; widthR = width;
            elseif strcmpi(args.Side,'both')
                widthL = width; widthR = width;
            else    % left
                widthL = width; widthR = 0;
            end
            width = (widthR + widthL) / 2;
            ...
            obj.MeanPlot = plot(pos+[-widthL,widthR]./width.*meanDensityWidth, ...
                                [meanValue, meanValue]);
            ...
            if ~isempty(lowhisker) && ~isempty(hiwhisker)
                obj.WhiskerPlot = plot([boxcntrpos boxcntrpos], [lowhisker hiwhisker]);
            end
            obj.MedianPlot = scatter(boxcntrpos, quartiles(2), [], [1 1 1], 'filled');

            obj.NotchPlots = ...
                 scatter(boxcntrpos, quartiles(2)-1.57*IQR/sqrt(length(data)), ...
                         [], [1 1 1], 'filled', '^');
            obj.NotchPlots(2) = ...
                 scatter(boxcntrpos, quartiles(2)+1.57*IQR/sqrt(length(data)), ...
                         [], [1 1 1], 'filled', 'v');
    ...
    methods (Access=private)
        function results = checkInputs(obj, data, pos, varargin)
            ...
            p.addParameter('Side', 'left', @ischar);
